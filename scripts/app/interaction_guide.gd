...
func resume() -> void:
    if _interaction.is_empty():
        return
    _enabled = true
    visual_hint_changed.emit(0.32 if _resume_boost_pending else 0.28)
