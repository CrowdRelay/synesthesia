...
func render(canvas, viewport_size: Vector2, progress: float, phase: float) -> void:
    ...
    canvas.draw_rect(bed, accent.with_alpha(0.04 + progress * 0.07), true)
    canvas.draw_line(bed.position, bed.position + Vector2(bed.size.x, 0.0), accent.with_alpha(0.10), 1.5)
    if progress > 0.34:
        ...
        canvas.draw_circle(first, 14.0, accent.with_alpha(alpha * 0.10))
        canvas.draw_circle(second + Vector2(sin(phase) * 2.0, 0.0), 14.0, secondary.with_alpha(alpha * 0.11))
