...
func render(canvas, viewport_size: Vector2, progress: float, phase: float) -> void:
    ...
    for index in range(5):
        ...
        var lateral: float = direction * sin(phase * 3.4 + float(index) * 0.7) * (10.0 + surge * 28.0)
        for sample in range(24):
            var x: float = viewport_size.x * float(sample) / 23.0 + lateral
            var wave: float = sin(float(sample) * 0.58 + phase * 4.8 + float(index))
            var cross: float = sin(float(sample) * 0.21 - phase * 3.0 + float(index))
            var y: float = baseline + float(index) * 16.0 + wave * (8.0 + progress * 17.0) + cross * surge * 8.0
            points.append(Vector2(x, y))
        canvas.draw_polyline(points, Color(accent, 0.09 + progress * 0.09 + surge * 0.04), 1.3 + float(index) * 0.25, true)
