...
static func draw_living_state(owner: Control, style: String, accent: Color, secondary: Color, alpha: float, live: float, t: float) -> void:
        ...
        match style:
            "technophobia":
                ...
                owner.draw_line(Vector2(owner.size.x * 0.12, y), Vector2(owner.size.x * 0.88, y), Color(accent, alpha * live * 0.28), 1.0)
                for i in range(3):
            ...
            "unmasked":
                ...
                for i in range(3):
                    ...
                    var start := Vector2(owner.size.x * (0.28 + i * 0.20), owner.size.y * (0.31 + i * 0.03))
            ...
            "invaluable":
                for i in range(8):
                    var a := float(i) * 0.83 + t * 0.08
                    var p := c + Vector2.from_angle(a) * owner.size.x * (0.12 + i % 3 * 0.05)
                    var glint := 0.35 + 0.65 * maxf(0.0, sin(t * 1.2 + i))
                    owner.draw_line(p - Vector2(5, 0), p + Vector2(5, 0), Color(accent, alpha * live * glint), 1.0)
            "seed":
                for i in range(16):
