...
static func apply_readability(material: ShaderMaterial, profile: Dictionary, high_readability: bool, viewport_size: Vector2) -> void:
    ...
    var local_contrast := clampf(float(profile.get("local_contrast", 0.34)) + high_gain * 0.12, 0.0, 1.0) * portrait_gain
    ...
    material.set_shader_parameter("readability_local_contrast", local_contrast)
