...
def complete_room(page: Page, expected_room: str | None, artifact_dir: pathlib.Path):
    ...
    previous_completion_count = page.evaluate("(window.__viryaSynE2EEvents || []).filter(x => x.phase === 'completion').length")
    ...
    while time.monotonic() < deadline:
        ...
        completion_count = page.evaluate("(window.__viryaSynE2EEvents || []).filter(x => x.phase === 'completion').length")
